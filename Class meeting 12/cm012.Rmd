...
Using Factors: area variable types taht permit storage of categorical data in R
  -Levels of factors are just values
  -in the background, levels are stored as integers. 
  ---
...
gapminder$country %>% 
  nlevels()
new_countries<- gapminder %>% 
filter(country==c("India", "Mexico","Canada", "Japan","Spain"))
lev<-gapminder %>% 
...
gapminder %>%
  ggplot() +
  geom_bar(aes(fct_rev(fct_infreq(continent)))) + #Fct_infreq used to sort the data!!
  coord_flip() +
  theme_bw() +
  ylab("Number of entries") + xlab("Continent")
```
...
gapminder %>%
  ggplot() +
  geom_bar(aes(fct_reorder(continent, lifeExp, max))) + #Fct_infreq used to sort the data!!
  coord_flip() +
  theme_bw() +
  ylab("Number of entries") + xlab("Continent")
```
...
gapminder %>%
  ggplot() +
  geom_bar(aes(fct_recode(continent, "OCN" ="Oceania", "AMR"="America"))) +
  coord_flip()+
  theme_bw() 
